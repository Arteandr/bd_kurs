...
function AuthPage() {
    ...
    const handleClick = (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        auth.signin(user)
        setMenuStatus(AccessStatus.Success, () => {
            navigate(from, {replace: true})
        })
    }
    ...
    return (
        <div id="auth">
            <div className='auth-menu' ref={authMenu}>
                <h1 className='auth-label'>Кинотеатры города-Рязанов Н.А-ПИ-21а</h1>
                <h1 className='auth-label'>Авторизация</h1>
                <div className='auth-form'>
                    <input id="auth-login" placeholder='Логин' onChange={(e) => setLogin(e.target.value)}
                           value={login}/>
                    <input id="auth-password" type={'password'} onChange={(e) => setPassword(e.target.value)}
